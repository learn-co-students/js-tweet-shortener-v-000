...
describe('TweetShortener', function() {
  ...
  it('replaces long words with their expected short form', function() {
    ...
    var temp = new TweetShortener();
    expect(temp.wordSubstituter(tweet)).toEqual(substituded_tweet);
  });
  ...
  it('shortens tweets in bulk', function(){
    ...
    var temp = new TweetShortener();
    expect(temp.bulkShortener(tweets)).toEqual(shortened_tweets);
  });
  ...
  it('shortens tweets longer than 140 characters only', function(){
    ...
    expect(temp.selectiveShortener(over140).length).toBeLessThan(over140.length);
    expect(temp.selectiveShortener(under140).length).toEqual(under140.length);
    expect(temp.selectiveShortener(over140Too).length).toBeLessThan(over140Too.length);
    expect(temp.selectiveShortener(under140Too).length).toEqual(under140Too.length);
  });
  ...
  it("truncates tweets over 140 characters after shortening", function(){
    ...
    var temp = new TweetShortener();
    var shortenedTweet = temp.shortenedTruncator(longTweet);
  ...
  it("keeps shortened tweets at their original lengths", function(){
    ...
    var temp = new TweetShortener();
    expect(temp.shortenedTruncator(shortTweet).length).toEqual(shortTweet.length);
  });
});
